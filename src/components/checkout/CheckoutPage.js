    ...
    <main className="pf-page pf-checkout">
      <div className="pf-checkout-inner">
        <header className="pf-checkout-head">
          ...
          <p className="pf-checkout-desc">
            본 화면은 포트폴리오용 결제 시뮬레이션입니다. 실제 결제는 진행되지 않으며, 결제 상태만 변경됩니다.
          </p>
        </header>
        ...
        <section className="pf-checkout-card">
          ...
          <div className="pf-pay-methods">
            ...
            <label className={`pf-radio ${payMethod === "KAKAO_PAY" ? "is-active" : ""}`}>
              <input
                ...
                onChange={() => setPayMethod("KAKAO_PAY")}
              />
              카카오페이
            </label>
          ...
          {payMethod !== "CARD" && (
            <p className="pf-hint">
              선택하신 결제 수단은 실제 연동되지 않습니다. “결제하기” 버튼을 통해 결제 상태만 시뮬레이션됩니다.
            </p>
          )}
        </section>

...
function Mypage() {
  ...
  const removeCartItem = (id, optionId) => {
    ...
    const next = cartItems.filter(
      (it) => !(String(it.id) === String(id) && String(it.optionId || "") === String(optionId || ""))
    );
    setCartItems(next);
    writeCart(next);
  };
      ...
      <div className="pf-mypage-inner">
        <header className="pf-head">
          ...
          <div className="pf-top-tabs">
            <button
              type="button"
              className={`pf-top-tab ${topSection === "experience" ? "is-active" : ""}`}
              onClick={goTopExperience}
            >
              체험
            </button>
            <button
              type="button"
              className={`pf-top-tab ${topSection === "store" ? "is-active" : ""}`}
              onClick={goTopStore}
            >
              스토어
            </button>
            ...
            <div className="pf-profile">
              <div className="pf-avatar">
                {accountForm.photo ? (
                  ...
                  <span>{String(baseDisplayName).slice(0, 1)}</span>
                )}
              </div>
            ...
            <nav className="pf-tabs">
              {topSection === "experience" && (
                <>
                  ...
                  <button
                    type="button"
                    className={`pf-tab ${tab === "bookmarks" ? "is-active" : ""}`}
                  ...
                  <button
                    type="button"
                    className={`pf-tab ${tab === "reviews" ? "is-active" : ""}`}
              ...
              {topSection === "store" && (
                <>
                  <button
                    type="button"
                    className={`pf-tab ${tab === "cart" ? "is-active" : ""}`}
                    onClick={() => setTab("cart")}
                  >
                    장바구니
                  </button>
                  ...
                  <button
                    type="button"
                    className={`pf-tab ${tab === "store_orders" ? "is-active" : ""}`}
                    onClick={() => setTab("store_orders")}
                  >
                    주문내역
                  </button>
                </>
              ...
              <button
                type="button"
                className={`pf-tab ${tab === "security" ? "is-active" : ""}`}
              ...
              <div className="pf-panel">
                <div className="pf-panel-head">
                  ...
                  <div className="pf-seg">
                    <button
                      type="button"
                      className={`pf-seg-btn ${resvFilter === "ALL" ? "is-active" : ""}`}
                      onClick={() => setResvFilter("ALL")}
                    >
                      전체
                    </button>
                    <button
                      type="button"
                      className={`pf-seg-btn ${resvFilter === "BOOKED" ? "is-active" : ""}`}
                      onClick={() => setResvFilter("BOOKED")}
                    >
                      진행중
                    </button>
                    <button
                      type="button"
                      className={`pf-seg-btn ${resvFilter === "DONE" ? "is-active" : ""}`}
                      onClick={() => setResvFilter("DONE")}
                    >
                      완료
                    </button>
                  </div>
                    ...
                    {filteredReservations.map((it) => {
                      ...
                      return (
                        <div className="pf-item" key={it.bookingId}>
                          <div className="pf-item-main">
                            ...
                            <div className="pf-item-actions">
                              <button
                                ...
                                onClick={() => navigate(`/list/${it.programId}`)}
                              >
                                상세
                              </button>
                              ...
                              {canRefund ? (
                                <button
                                  type="button"
                                  className="pf-linkbtn"
                                  onClick={() => handleRefundReservation(it.bookingId)}
                                >
                                  결제 취소
                                </button>
                              ) : null}
                              ...
                              {canCancel ? (
                                <button
                                  type="button"
                                  className="pf-linkbtn"
                                  onClick={() => handleCancelReservation(it.bookingId)}
                                >
                                  예약 취소
                                </button>
                              ) : null}
                          ...
                          {/* ✅ 상태칩 */}
                          <span className={`pf-chip ${statusClass(it.status, it.paymentStatus)}`}>
                            {chipText(it.status, it.paymentStatus)}
                          </span>
                        </div>
                      );
                    })}
              ...
              <div className="pf-panel">
                <div className="pf-panel-head">
                  ...
                  <div className="pf-cart-head-actions">
                    <button
                      type="button"
                      className={`pf-ghost ${cartItems.length === 0 ? "is-disabled" : ""}`}
                      onClick={clearCart}
                          ...
                          <div className="pf-item" key={`${it.id}_${it.optionId || "noopt"}`}>
                            <div className="pf-item-main">
                              ...
                              <p className="pf-item-sub">
                                ...
                                단가 {Number(it.price || 0).toLocaleString()}원{" · "}합계{" "}
                              ...
                              <div className="pf-item-actions">
                                ...
                                <div className="pf-qty">
                                  <button
                                    ...
                                    onClick={() => updateCartQty(it.id, it.optionId, Number(it.qty || 1) - 1)}
                                  >
                                    −
                                  </button>
                                  <input
                                ...
                                <button
                                  type="button"
                                  className="pf-linkbtn"
                                  onClick={() => removeCartItem(it.id, it.optionId)}
                                >
                                  삭제
                                </button>
                              </div>
                            ...
                            {/* {items.length > 0 && (
                              <div style={{ marginTop: 8 }}>
                                {items.slice(0, 3).map((it, idx) => (
                                  <div
                                    key={`${o.orderId}_${idx}`}
                                    style={{ fontSize: 13, color: "var(--color-list-sub)" }}
                                  >
                                    - {it.title} {it.optionName ? `(${it.optionName})` : ""} · {it.qty}개
                                  </div>
                                ))}
                                {items.length > 3 ? (
                                  <div style={{ fontSize: 13, color: "var(--color-list-sub)" }}>
                                    … 외 {items.length - 3}개
                                  </div>
                                ) : null}
                              </div>
                          ...
                          <div className="pf-item-main">
                            ...
                            <div className="pf-item-actions">
                              <button
                                type="button"
                                className="pf-linkbtn"
                                onClick={() => navigate(`/list/${programId}`)}
                              >
                                상세
                              </button>

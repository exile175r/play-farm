                        ...
                        <div className="pf-item" key={it.bookingId}>
                          <div className="pf-item-main">
                            ...
                            <div className="pf-item-actions">
                              ...
                              {/* ✅ 결제하기 버튼 (경로 수정: /checkout + state로 bookingId 전달) */}
                              <button
                                type="button"
                                className={`pf-linkbtn ${needPay ? "" : "is-disabled"}`}
                                onClick={() => {
                                  if (!needPay) return;
                                  navigate("/checkout", { state: { bookingId: it.bookingId } });
                                }}
                                disabled={!needPay}
                          ...
                          {/* ✅ 상태칩: 예약상태 + 결제상태 같이 보이게(포폴용) */}
                          <span className={`pf-chip ${statusClass(it.status)}`}>
                            {statusLabel(it.status)}
                            {it.status === "BOOKED" ? ` · ${paymentLabel(it.paymentStatus)}` : ""}
                            {it.status === "CANCELLED" ? ` · ${paymentLabel(it.paymentStatus)}` : ""}
                          </span>
                        </div>
                      );
                    })}

      ...
      <main className="sod-wrap">
        <div className="sod-inner">
          ...
          <button
            className="sod-btn"
            type="button"
            onClick={() => navigate("/mypage", { state: { openTab: "store_orders" } })}
          >
            주문내역으로 이동
          </button>
        </div>
    ...
    <main className="sod-wrap">
      <div className="sod-inner">
        <div className="sod-head">
          ...
          <div
            className={`sod-chip ${
              order.status === "PAID" ? "is-ok" : order.status === "REFUNDED" ? "is-refund" : "is-cancel"
            }`}
          >
        ...
        <section className="sod-card">
          ...
          {items.length === 0 ? (
            ...
            <ul className="sod-items">
              {items.map((it, idx) => {
                const line =
                  Number(it.unitPrice ?? it.unitPrice === 0 ? it.unitPrice : it.unitPrice) || Number(it.unitPrice || 0);
                const unitPrice =
                  Number(it.unitPrice || it.unitPrice === 0 ? it.unitPrice : it.unitPrice) || Number(it.unitPrice || 0);
                ...
                const lineTotal =
                  (Number(it.unitPrice || it.unitPrice === 0 ? it.unitPrice : it.unitPrice) ||
                ...
                const safeLineTotal =
                  (Number(it.unitPrice || 0) ||
                    ...
                    ? (Number(it.unitPrice || 0) || Number(it.unitPrice || 0) || Number(it.unitPrice || 0)) *
                      Number(it.qty || 0)
                    : Number(it.unitPrice || 0) * Number(it.qty || 0);

                // ✅ CheckoutPage에서 items에 unitPrice로 저장했으니 대부분 정상
                const finalLineTotal =
                  (Number(it.unitPrice || 0) || Number(it.unitPrice || 0) || 0) * Number(it.qty || 0);
                ...
                return (
                  <li key={`${order.orderId}_${idx}`} className="sod-item">
                    ...
                    <div className="sod-item-right">
                      ...
                      <div className="sod-item-total">
                        {(Number(it.unitPrice || 0) * Number(it.qty || 0)).toLocaleString()}원
                      </div>
                    </div>
                  </li>
                );
        ...
        <div className="sod-actions">
          ...
          <button
            type="button"
            className={`sod-btn ${canCancel ? "" : "is-disabled"}`}
            onClick={onCancel}
            disabled={!canCancel}
          >
            주문 취소
          ...
          <button
            type="button"
            className={`sod-btn danger ${canRefund ? "" : "is-disabled"}`}
            onClick={onRefund}
            disabled={!canRefund}
          >
            환불 처리

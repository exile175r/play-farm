      ...
      return data.map((item) => {
         ...
         try {
            if (typeof newItem.program_nm === 'string' && newItem.program_nm.includes(' 체험')) {
               // 이미 보기 좋으면 그대로
            } else if (typeof newItem.program_nm === 'string') {
               newItem.program_nm = newItem.program_nm.replace(/체험|및/g, (match) => replaceText[match] || match);
            } else {
               const parsed = Array.isArray(newItem.program_nm) ? newItem.program_nm : JSON.parse(newItem.program_nm);
               newItem.program_nm = parsed.map((v) => v.replace(/체험|및/g, (match) => replaceText[match] || match)).join(', ');
            }
         } catch {

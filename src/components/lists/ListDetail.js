  ...
  const fetchProgramDetail = async (id) => {
    try {
      ...
      if (result.success) {
        ...
        try { result.data.program_nm = JSON.parse(result.data.program_nm).map(v => v.replace(/체험|및/g, match => replaceText[match])).join(', '); }
        catch (error) { result.data.program_nm = result.data.program_nm.replace(/체험|및/g, match => replaceText[match]); }
            ...
            <div className="detail-top">
              ...
              <div className="detail-info">
                ...
                <div className="detail-main-text">
                  ...
                  <p>신청 기간 : {dayjs(data.reqst_bgnde).format('YYYY.MM.DD') || "?"} ~ {dayjs(data.reqst_endde).format('YYYY.MM.DD') || "?"}</p>
